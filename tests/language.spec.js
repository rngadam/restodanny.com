...
test.describe("Language Navigation and Detection", () => {
  test.describe("Standard Desktop", () => {
    ...
    test("Default language is French", async ({ page }) => {
      ...
      await expect(
        page.locator("a", { hasText: "L'Histoire" }).first(),
      ).toBeVisible();
    });
    ...
    test("Language selector works (FR to EN to FR)", async ({ page }) => {
      ...
      await expect(
        page.locator("a", { hasText: "The Story" }).first(),
      ).toBeVisible();
    ...
    test.describe("With English Locale", () => {
      ...
      test("Auto-redirects to English if browser language is English", async ({
        page,
      }) => {
    ...
    test("Does not auto-redirect if user previously selected French", async ({
      page,
    }) => {
  ...
  test.describe("Mobile Viewport", () => {
    ...
    test("Language selector works on mobile", async ({ page }) => {
      ...
      await page.evaluate(() => {
        ...
        for (let i = 0; i < links.length; i++) {
          if (
            links[i].textContent === "EN" &&
            links[i].closest(".-mr-2.flex")
          ) {
            links[i].click();
            break;
          }
